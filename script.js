document.addEventListener('DOMContentLoaded', () => {
  ...
  function renderDiaryList(){
    ...
    diaries.forEach(diary=>{
      ...
      if(diary.image){ const imgEl=document.createElement('img'); imgEl.src=diary.image; imgEl.style.maxHeight='60px'; imgEl.style.maxWidth='100%'; thumb.appendChild(imgEl); }
      else thumb.textContent='썸네일';
      const titleEl=document.createElement('div'); titleEl.textContent=diary.title; titleEl.style.fontSize='12px';
      const dateEl=document.createElement('div'); dateEl.textContent=diary.date; dateEl.style.fontSize='10px'; dateEl.style.color='#888';
      item.appendChild(thumb); item.appendChild(titleEl); item.appendChild(dateEl);
